...
def filter_transitions(df, desired_transitions = ['n_to_pi_*_L', 'n_to_pi_*_R', 'pi_nb_to_pi_*_R', 'pi_nb_to_pi_*_L']):
    # filter the transitions that have a energy difference of less than 0.01 eV
    df = df.copy()
    transitions_df = df[df['transition'].isin(desired_transitions)]
    ground_state_rows = df[df['end_state'] == 1]
    ground_state_rows.loc[:, 'transition'] = 'GROUND'
    transitions_df = pd.concat([transitions_df, ground_state_rows])
    transitions_df = transitions_df.sort_values(by=['phi', 'psi', 'end_state'])
